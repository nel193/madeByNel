...
const Pixels3 = () => {
  ...
  for (let i = 0; i < particlesMatrix.length; i++) {
    const tmpArr = particlesMatrix[i] = [];
  ...
  class Particle {
    ...
    cleanPreviousDraw() {
      ...
      for (let i = 0; i < doubleSize; i++) {
        const rowIndex = colorIndex + i * imageWidth4;
    ...
    update() {
      ...
      if (distance < mouseRadius + this.size) {
        ...
        const directionX = (forceDirectionX * forceTimesDensity);
        const directionY = (forceDirectionY * forceTimesDensity); //Ch.this
        this.x -= directionX;
        this.y -= directionY;
        redraw = true;
      } else {
  ...
  function initializeCanvas(ctx, canv) {
    ...
    for (let i = 0; i < imageData.data.length; i++) {
      imageData.data[i] = (i + 1) % 5 ? 0 : 0; //R: 0, G: 0, B: 0, A: 255
    }

    let particles = [];

    function drawImage() {
      const imageWidth = png.width; //These to values crop if / sum no.
      ...
      for (let y = 0, y2 = srcImageData.height; y < y2; y += pixelStep) {
        for (let x = 0, x2 = srcImageData.width; x < x2; x += pixelStep) {
          const colorIndex = (y * 4 * srcImageData.width) + (x * 4);
          const alphaColor = srcImageData.data[colorIndex + 3];
          ...
          if (alphaColor >= colorAlphaTreshold) {
            ...
            if (color.r >= colorTreshold || color.g >= colorTreshold || color.b >= colorTreshold) {
              const particle = new Particle(x * zoomLevel, y * zoomLevel, color, imageData);
